...
#First Divided Train & Test in 80:20, then divided Train & Validation in 75:25 (6/8 = X/100)
#训练集是为了进行模型的训练，验证集是为了进行参数的调整，测试集是为了看这个模型的好坏。
...
#fit不是一个训练的过程，而是一个适配的过程，过程都是确定的，最后得到一个可用于转换的有价值的信息
#fit计算训练数据的均值和方差，后面用均值和方差来转换训练数据
#fit为后续API服务； fit后，调用各种API方法//transform只是其中一个API方法

trainScoreList = []
validScoreList = []

#trainScore = np.zeros(len(KList))
#validScore  = np.zeros(len(KList))

#clf_KNN = KNeighborsClassifier(n_neighbors = 10)
#clf_KNN.fit(cancer_X_train, cancer_y_train)

#cross_validation: 进行模型的性能度量, 输出每个样本的预测结果
#cancer_pred_train = cross_val_predict(clf_KNN, cancer_X_train, cancer_y_train)
#cancer_pred_train
#cancer_pred_validation = cross_val_predict(clf_KNN, cancer_X_validation, cancer_y_validation)
#cancer_pred_test = cross_val_predict(clf_KNN, cancer_X_test, cancer_y_test)
...
#cancer_pred_train = cross_val_predict(clf_NB,cancer_X_train, cancer_y_train)
#cancer_pred_validation = cross_val_predict(clf_NB,cancer_X_validation, cancer_y_validation)
#cancer_pred_test = cross_val_predict(clf_NB,cancer_X_test, cancer_y_test)
...
#trainScore = np.zeros(len(alphaList))
#validScore  = np.zeros(len(alphaList))

trainScoreList = []
validScoreList = []

#cancer_pred_train = cross_val_predict(clf_lasso,cancer_X_train, cancer_y_train)
#cancer_pred_validation = cross_val_predict(clf_lasso,cancer_X_validation, cancer_y_validation)
#cancer_pred_test = cross_val_predict(clf_lasso,cancer_X_test, cancer_y_test)
...
#Confusion_matrix(y_true, y_pred)
#lasso回归没有直接得到binary variable, 需要用0.5为阈值进行转换
print("----- Train confusion matrix-----")
...
#cancer_pred_train = cross_val_predict(clf_NN,cancer_X_train, cancer_y_train)
#cancer_pred_validation = cross_val_predict(clf_NN,cancer_X_validation, cancer_y_validation)
#cancer_pred_test = cross_val_predict(clf_NN,cancer_X_test, cancer_y_test)
...
#clf_NN = MLPClassifier(solver='lbfgs', hidden_layer_sizes=(10,5,3))
#clf_NN.fit(cancer_X_train, cancer_y_train)
#print(clf_NN.score(cancer_X_test, cancer_y_test['Survived']))
...
#age_standard_scaler = preprocessing.StandardScaler()
#fare_standard_scaler = preprocessing.StandardScaler()
#titanic_train_data['Age'] = age_standard_scaler.fit_transform(titanic_train_data['Age'].values.reshape(-1,1))
#titanic_test_data['Age'] = age_standard_scaler.transform(titanic_test_data['Age'].values.reshape(-1,1))
#titanic_train_data['Fare'] = fare_standard_scaler.fit_transform(titanic_train_data['Fare'].values.reshape(-1,1))
#titanic_test_data['Fare'] = fare_standard_scaler.transform(titanic_test_data['Fare'].values.reshape(-1,1))



#%%% Split Data
#划分数据集，其中train按照 80/20分为train和validation
#数据集中的test没有survived变量, 只有X_test, 没有y_test
#titanic_X_train, titanic_X_test, titanic_y_train, titanic_y_test = train_test_split(titanic_train_data['Survived'], test_size = 0.2, random_state = 42)
#titanic_X_train, titanic_X_validation, titanic_y_train, titanic_y_validation = train_test_split(titanic_X_train, titanic_y_train, test_size = 0.25, random_state = 42)
...
#cross_validation: 进行模型的性能度量, 输出每个样本的预测结果
#titanic_pred_train = cross_val_predict(clf_KNN,titanic_X_train, titanic_y_train)
#titanic_pred_train
#titanic_pred_validation = cross_val_predict(clf_KNN,titanic_X_validation, titanic_y_validation)
#titanic_pred_validation
#titanic_pred_test = cross_val_predict(clf_KNN, cancer_X_test, cancer_y_test)
#titanic_pred_test
...
#titanic_pred_train = cross_val_predict(clf_NB,titanic_X_train, titanic_y_train)
#titanic_pred_validation = cross_val_predict(clf_NB,titanic_X_validation, titanic_y_validation)
#titanic_pred_test = cross_val_predict(clf_NB, titanic_X_test, titanic_y_test)
...
#titanic_pred_train = cross_val_predict(clf_lasso,titanic_X_train, titanic_y_train)
#titanic_pred_train 
#titanic_pred_validation = cross_val_predict(clf_lasso,titanic_X_validation, titanic_y_validation)
#titanic_pred_validation
#titanic_pred_test = cross_val_predict(clf_lasso, titanic_X_test, titanic_y_test)
#titanic_pred_test 
...
#_pred_train = cross_val_predict(clf_NN,titanic_X_train, titanic_y_train)
#titanic_pred_validation = cross_val_predict(clf_NN, titanic_X_validation, titanic_y_validation)
...
#%%% Summary Titanic
#Accuracy Comparison among all the predictive models
print(accuracy_df)
...
#clf_NN = MLPClassifier(solver='lbfgs', hidden_layer_sizes=(10,5,3))
#clf_NN.fit(titanic_X_train, titanic_y_train)
#print(clf_NN.score(titanic_X_test, titanic_y_test['Survived']))
